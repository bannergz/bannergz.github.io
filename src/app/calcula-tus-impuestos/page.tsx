...
export default function CalculaTusImpuestosPage() {
  return (
    <div lang="es">
        ...
        <div className="section-container relative z-10 pt-32 pb-14">
          <div className="mx-auto max-w-4xl">
            ...
            <h1 className="text-4xl font-bold leading-tight tracking-tight text-white md:text-5xl lg:text-6xl">
              ¿Cuánto me descuenta <span className="text-accent-light">SUNAT</span>?
            </h1>
            ...
            <div className="mt-12 grid grid-cols-2 gap-8 border-t border-white/10 pt-10 md:grid-cols-4">
              {cifras.map((c) => (
                <div key={c.etiqueta}>
                  <p className="whitespace-nowrap text-2xl font-bold tabular-nums text-white lg:text-3xl">{c.valor}</p>
                  <p className="mt-1 text-sm text-gray-400">{c.etiqueta}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <dl className="grid gap-x-10 gap-y-8 md:grid-cols-2">
            {preguntasFrecuentes.map((f) => (
              <div key={f.pregunta} className="flex flex-col gap-2">
                <dt className="text-lg font-semibold text-primary">{f.pregunta}</dt>
      ...
      {/* ---------------- fuentes ---------------- */}
      <section id="fuentes" className="bg-surface-dark">
        <div className="section-container">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {fuentesOficiales.map((f) => (
              ...
              >
                <span className="font-semibold text-primary">{f.titulo}</span>
          ...
          <div className="mt-12 max-w-3xl space-y-3 border-t border-gray-200 pt-8 text-sm leading-relaxed text-text-muted">
            <p>
              <b className="font-semibold text-primary">Referencia, no asesoría.</b> El cálculo aplica
            ...
            <p>
              <b className="font-semibold text-primary">Lo que no cubre.</b> Rentas de primera,
